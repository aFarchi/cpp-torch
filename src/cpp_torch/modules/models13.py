...
class ProgressiveDecoder(nn.Module):
    def __init__(self, latent_dim: int, hidden_dims: List[int], out_dim: int,
        ...
        self.expand_norm = LayerNorm(all_dims[1])
        ...
        self.output_proj = nn.Sequential(
            nn.Linear(all_dims[-2], all_dims[-2]),
            nn.GELU(approximate='tanh'),
            nn.Linear(all_dims[-2], out_dim),
        )
        self.output_residual = (ResidualBlock(all_dims[-2], out_dim, feature_dropout * 0.5)
                                if use_residualsIO else None)

        self.unpoolers = nn.ModuleDict()
        # FiLM conditioning on static fields (orography, land-sea mask,
        # position) of the destination level, applied right after each unpool.
        # Zero-initialised: identity at init, checkpoint-friendly.
        self.static_films = nn.ModuleDict()
        for unpool_step, layer_idx in enumerate(self.unpooling_schedule):
            in_f      = all_dims[layer_idx]
            unpool_ea = (interlevel_edge_attrs[unpool_step]
                         if interlevel_edge_attrs is not None else None)
            self.unpoolers[str(layer_idx)] = AttentionUnpooling(
                level_connections[unpool_step], in_dim=in_f, num_heads=heads,
                edge_attr=unpool_ea)

            # the unpool_step-th unpool (in forward order) lands on this level
            dest_level = len(graphs) - 2 - unpool_step
            sf = (static_feats[dest_level]
                  if static_feats is not None and dest_level < len(static_feats)
                  else None)
            if sf is not None:
                self.register_buffer(f'static_feats_{layer_idx}', sf)
                film = nn.Linear(sf.shape[-1], in_f * 2)
                nn.init.zeros_(film.weight)
                nn.init.zeros_(film.bias)
                self.static_films[str(layer_idx)] = film

    def _apply_static_film(self, x: torch.Tensor, layer_idx: int) -> torch.Tensor:
        key = str(layer_idx)
        if key in self.static_films:
            sf = getattr(self, f'static_feats_{layer_idx}')
            scale, shift = self.static_films[key](sf).chunk(2, dim=-1)
            x = x * (1 + scale) + shift
        return x
    ...
    def forward(self, latent: torch.Tensor) -> torch.Tensor:
        ...
        x = F.gelu(self.expand_norm(x), approximate='tanh')
        x = F.dropout(x, p=self.feature_dropout, training=self.training)
        ...
        for i, blk in enumerate(self.gat_blocks):
            ...
            if layer_idx in self.unpooling_schedule and unpool_step >= 0:
                x = self.unpoolers[str(layer_idx)](x)
                x = self._apply_static_film(x, layer_idx)
                unpool_step -= 1
                graph_level -= 1
    ...
    def decode_linear(self, latent: torch.Tensor) -> torch.Tensor:
        """Approximate tangent-linear decoder: MLP sub-blocks in GATBlocks removed.

        WARNING: validated on v81 to differ from the full decoder's Jacobian
        by ~48% relative L2 error (cosine 0.88) — prefer the exact full
        decoder TL/adjoint (decode_tl / decode_adjoint on the autoencoder).

        Intended use (DA inner loop):
            _, Jdz = torch.func.jvp(model.decode_linear, (z_b,), (dz,))
            _, vjp_fn = torch.func.vjp(model.decode_linear, z_b)
            JT_lam = vjp_fn(lam)[0]
        """
        graph_level  = len(self.graphs) - 1
        unpool_step  = len(self.unpooling_schedule) - 1
        ...
        for i, blk in enumerate(self.gat_blocks):
            ...
            edge_idx = self.graphs[graph_level]
            ea       = getattr(self, f'edge_attr_{graph_level}') if self._n_edge_attrs > 0 else None
            x = blk.forward_linear(x, edge_idx, ea)
    ...
    def decode_linear_ckpt(self, latent: torch.Tensor) -> torch.Tensor:
        """Memory-safe variant of decode_linear with activation checkpointing.

        torch.utils.checkpoint does not compose reliably with torch.func.vjp,
        so use plain autograd for the adjoint:
            z = z_b.detach().requires_grad_()
            y = model.decode_linear_ckpt(z)
            JT_lam = torch.autograd.grad(y, z, grad_outputs=lam)[0]
        """
        graph_level  = len(self.graphs) - 1
        unpool_step  = len(self.unpooling_schedule) - 1
        ...
        for i, blk in enumerate(self.gat_blocks):
            ...
            edge_idx = self.graphs[graph_level]
            ea       = getattr(self, f'edge_attr_{graph_level}') if self._n_edge_attrs > 0 else None
            x = checkpoint(blk.forward_linear, x, edge_idx, ea, use_reentrant=False)
...
class ProgressiveGraphAutoencoder(nn.Module):
    ...
    Identical to models12.ProgressiveGraphAutoencoder except:
      ...
      * Exact full-decoder TL/adjoint for DA: decode_tl (forward-mode jvp via
        ...
        through the checkpointed forward).  Preferred over decode_linear,
        whose Jacobian was measured ~48% off the full decoder's on v81.
      * decode_linear_ckpt: checkpointed linearised decode for a memory-safe
        adjoint via torch.autograd.grad (kept for comparison).
    """
    ...
    # (decode_linear's Jacobian was measured ~48% off the full decoder's
    # on v81; the exact operators below avoid that approximation.)
    ...
    def decode_adjoint(self, z_b: torch.Tensor, lam: torch.Tensor) -> torch.Tensor:
        ...
        Memory-safe: reverse-mode through the checkpointed forward.
    ...
    def decode_linear(self, latent: torch.Tensor) -> torch.Tensor:
        """Approximate tangent-linear decode (MLP sub-blocks removed).

            _, Jdz = torch.func.jvp(model.decode_linear, (z_b,), (dz,))
            _, vjp_fn = torch.func.vjp(model.decode_linear, z_b)
            JT_lam = vjp_fn(lam)[0]
        """
        return self.decoder.decode_linear(latent)

    def decode_linear_ckpt(self, latent: torch.Tensor) -> torch.Tensor:
        """Checkpointed linearised decode — memory-safe adjoint via autograd:

            z = z_b.detach().requires_grad_()
            y = model.decode_linear_ckpt(z)
            JT_lam = torch.autograd.grad(y, z, grad_outputs=lam)[0]
        """
        return self.decoder.decode_linear_ckpt(latent)
